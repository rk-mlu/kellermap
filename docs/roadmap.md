...
**WP 1** measured the bottleneck of SYM-7 and is done. It built no step type,
changed no `verify()` and added no check, so it took no identifier; what it
owed `docs/contracts.md` was one amendment to the wording of SYM-7, stated
...
density or the method, none of which was isolated. What was found instead is a
fifth thing the plan did not have: the presence or absence of a carrier, which
is a property of the step rather than of the size of its target.

**What was tried and did not work, which is most of the package.**

The first version measured `sp.Matrix(target.jacobian()).det()` for three
rounds. That is the public boundary and not the computation:
`PolynomialMap.determinant()` has computed in the polynomial ring since 0.5 and
takes the Schur complement first. Every figure from those rounds is withdrawn,
along with the four factors the specification was built on, which were factors
of the wrong object. The question that should have opened the package -- which
code does this obligation run -- is now the first line of the specification for
any package of this kind.

A different acyclicity rule doubles the carrier on all three lifts: breaking
each cycle at its busiest vertex leaves 17 of 38 rather than 9, and 20 of 40
and 21 of 44. It buys nothing. The complement narrows from 29 by 29 to 21 by
...
`k` submatrix cancels not at all: four by four is over three minutes, a
smaller matrix with a far bigger answer. The synthetic ladder built for the
old specification fails the same way and so does substituting a rational
constant for `i`, which was withdrawn earlier for what turns out to be the
same reason. Any probe that does not preserve the cancellation measures a
different problem, and none that preserves it is cheap.

**The amendment the package owed `docs/contracts.md` is made.** SYM-7's second
paragraph named dimension six against dimension forty and read as a statement
about size. It names the carrier now, says what was separated and says plainly
that the residual was not decomposed further.

**What a later package could try.** The answer is one and the intermediates are
not, which is the case for an evaluation-and-interpolation or a modular
determinant rather than the fraction-free one `DomainMatrix` chooses. That is
optimization and this package optimized nothing.

### Why the order

WP 1 ran before WP 2 although WP 2 is the more important of the two. The
reason given was that WP 2 makes the chain longer and a determinant that does
not finish at 40 variables will not finish at whatever the refinement
produces. That reasoning was wrong in its premise and right in its
conclusion. The cost does not follow the length of the chain: it follows
whether the target carries a unipotent block, and every stage but the lift
carries one at any dimension. So the refinement will not make the determinant
worse than it is, and it will not make it better either.
